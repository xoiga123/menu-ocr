...
from final_processing import get_result
...
class MMOCR:
    ...
    def __init__(self,
        ...
        kie_models = {
            'SDMGR': {
                ...
                'sdmgr/sdmgr_novisual_60e_wildreceipt_openset_20220803-427ec7a8.pth'
                # 'sdmgr/sdmgr_novisual_60e_wildreceipt_20220803-d06d4a1a.pth'
            }
        }
        ...
        # config['device'] = 'cpu'
        # config['weights'] = '/khoinlg_menu/model_weights/hub/checkpoints/transformerocr_seq.pth' 
        # self.recog_model = Predictor(config)
        
        # this for API
        self.recog_model = vietocr
        ...
        if self.kie_model:
            self.kie_dataset = KIEDataset(
                dict_file=self.kie_model.cfg.data.test.dict_file)

        # Attribute check
        # for model in list(filter(None, [self.recog_model, self.detect_model])):
        for model in list(filter(None, [self.detect_model])):
            if hasattr(model, 'module'):
                model = model.module

        self.ori_img_meta = None
    ...
    def readtext(self,
        ...
        self.args = args
        self.ori_img_meta = [img, image_name]
        pp_result = None
        ...
        # return pp_result
        return get_result(self.ori_img_meta, pp_result)
    ...
    def det_recog_kie_inference(self, det_model, recog_model, kie_model=None):
        ...
        # print('before det_model.forward')
        # t1 = time.time()
        ...
        for filename, arr, bboxes, out_file in zip(self.args.filenames,
            ...
            new_box = []
            for bbox in bboxes:
                new_box.append({'box': [round(x) for x in bbox[:-1]]})
            new_box = stitch_boxes_into_lines(
                new_box, self.args.merge_xdist, 0.5)
            # print('stitch', time.time() - t1)
            for bbox in new_box:
                box_res = {}
                box_res['box'] = bbox['box']
                # box_res['box_score'] = float(bbox[-1])
                box = box_res['box']
                # print(box)
                if len(bbox) > 9:
                    print("HAVE NOT ACCOUNTED FOR THIS LOL")
                    exit()
            ...
            if self.args.batch_mode:
                ...
                recog_results = self.single_inference(
                    recog_model, box_imgs, True, self.args.recog_batch_size, vietocr=True)
    ...
    def single_inference(self, model, arrays, batch_mode, batch_size=0, vietocr=False):
        # print("ocr", vietocr)
        def inference(m, a, **kwargs):
            if model == 'Tesseract_det':
                ...
                return model_inference(m, a, vietocr=vietocr, **kwargs)
    ...
    def _args_processing(self, args):
        ...
        args.arrays = [mmcv.imfrombytes(x, backend='cv2') for x in img_list]
        ...
        args.filenames = ['' for x in range(len(img_list))]
