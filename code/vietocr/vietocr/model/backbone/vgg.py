...
class Vgg(nn.Module):
    def __init__(self, name, ss, ks, hidden, pretrained=True, dropout=0.5):
        ...
        if name == 'vgg11_bn':
            cnn = models.vgg11_bn(pretrained=False)  # edit
        elif name == 'vgg19_bn':
            cnn = models.vgg19_bn(pretrained=False)  # edit
        cnn.load_state_dict(torch.load('/khoinlg_menu/model_weights/hub/checkpoints/vgg19_bn-c79401a0.pth'))
